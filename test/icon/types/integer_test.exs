defmodule Icon.Types.IntegerTest do
  ...
  describe "type/0" do
    test "it's an integer" do
      assert :integer = Icon.Types.Integer.type()
    end
  end

  describe "cast/1" do
    test "when an integer is provided, returns said integer" do
      assert {:ok, 42} = Icon.Types.Integer.cast(42)
    end

    test "when a valid hex string is provided, returns the equivalent integer" do
      assert {:ok, 42} = Icon.Types.Integer.cast("0x2A")
      assert {:ok, 42} = Icon.Types.Integer.cast("0x2a")
    end
    ...
    test "when an integer is negative, errors" do
      assert :error = Icon.Types.Integer.cast(-42)
    end

    test "when an invalid hex string is provided, errors" do
      assert :error = Icon.Types.Integer.cast("0xINVALID")
    end

    test "when an invalid string is provided, errors" do
      assert :error = Icon.Types.Integer.cast("INVALID")
    end

    test "when an unsupported type is provided, errors" do
      assert :error = Icon.Types.Integer.cast(nil)
      assert :error = Icon.Types.Integer.cast(:atom)
      assert :error = Icon.Types.Integer.cast(%{})
      assert :error = Icon.Types.Integer.cast([])
    end
  end

  describe "load/1" do
    test "when a valid hex is provided, returns the equivalent integer" do
